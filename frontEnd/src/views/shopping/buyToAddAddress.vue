  ...
  <div class="buyToAddAddress">
    ...
    <div class="buyToAddAddress-main">
      ...
      <template v-for="(item, index) in productItems">
        <div class="product-items" :key="index">
          <van-card
            :num="item.number"
            :price="item.price"
            :title="item.title"
          ...
          <div class="items-bottom">
            <p>共 {{ item.number }} 件 <span>小计：</span><span class="red">￥{{ item.totalPrice }}</span></p>
          </div>
        </div>
      </template>
    </div>
...
export default {
  ...
  methods: {
    ...
    submitOrders () {
      ...
      this.$store.dispatch('shoppingcat/toBuyProducts', this.address).then(res => {
        if (res.code === 1) {
  ...
  computed: {
    ...
    productItems () {
      let arr = []
      this.shoppingCat.forEach(ele => {
        if (this.selec.indexOf(ele.id) > -1) {
          arr.push(ele)
        }
      })
      return arr
