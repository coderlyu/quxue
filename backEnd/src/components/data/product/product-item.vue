          ...
          <div class="bg-white margin10 padd10 borderra4 flex-dc">
            <div :class="isMiniWid ? 'column-n1' : 'column-n2'" >
              <div style="height:100px;">
                <img v-if="item.avatar" :src="item.avatar" alt="头像" style="width:80px;height:80px;border-radius:50%;">
              ...
              <p class="name margin5">商品名 - <span class="color0" @click="toDetail(item)">{{ item.name }}</span></p>
              <p class="margin5">价格 - <span class="price">￥ {{ item.price }}</span></p>
              <p class="address margin5">简介 - {{ item.info ? item.info : '这么好的商品不来个介绍吗？' }}</p>
            </div>
